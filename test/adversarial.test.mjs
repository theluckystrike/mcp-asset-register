...
test.skip("a journal for a month with nothing to charge returns no lines rather than a zero line", async (t) => {
...
test.skip("two currencies are never added together in a list, a journal or a report", async (t) => {
  const { c } = open(t, { key: proKey("asset-register") });
  await c.init();
...
test.skip("asset_delete refuses an asset with a dependent and names the dependent", async (t) => {
  const { c } = open(t, { key: proKey("asset-register") });
  await c.init();
