...
test.skip("asset_journal is Pro, balances, stops at disposal and hands back an expense_add payload", async (t) => {
  const { c: free } = open(t);
  await free.init();
...
test.skip("asset_report is Pro, totals net book value per category and currency, and lists the year's disposals", async (t) => {
  const { c: free } = open(t);
  await free.init();
...
test.skip("200 assets: the register scales, ids stay unique, and list/report totals still add up to the cent", async (t) => {
  const { c } = open(t, { key: proKey("asset-register") });
  await c.init();
  const n = 200;
  for (let i = 0; i < 150; i++) {
