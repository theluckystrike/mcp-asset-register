...
test.skip("a corrupt assets.json is quarantined byte for byte and every later call fails", async (t) => {
  const box = sandbox();
...
test.skip("a corrupt counter.json blocks the write and leaves the register readable", async (t) => {
  const box = sandbox();
  const c = client({ dataHome: box.dataHome, key: proKey("asset-register") });
  t.after(() => { c.close(); cleanup(box.dir); });
  await c.init();
